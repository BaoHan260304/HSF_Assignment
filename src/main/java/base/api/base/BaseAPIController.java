...
public abstract class BaseAPIController {
    ...
    protected <T> ResponseEntity<TFUResponse<T>> success(T data, String message) {
        TFUResponse<T> body = new TFUResponse<>(true, data, message, HttpStatus.OK.value(), null);
    ...
    protected <T> ResponseEntity<TFUResponse<T>> error(String message, HttpStatus status) {
        TFUResponse<T> body = new TFUResponse<>(false, null, message, status.value(), null);
